...
function upload-resources() {
  swift post "${SWIFT_BUCKET_NAME}" --read-acl '.r:*,.rlistings'
  ...
  swift upload "${SWIFT_BUCKET_NAME}" ${RELEASE_TAR_PATH}/${KUBERNETES_RELEASE_TAR} \
    --object-name kubernetes-server.tar.gz

  echo "[INFO] Uploading kubernetes-salt.tar.gz"
  swift upload "${SWIFT_BUCKET_NAME}" ${RELEASE_TAR_PATH}/kubernetes-salt.tar.gz \
...
function run-heat-script() {
  ...
  local swift_repo_url="${SWIFT_SERVER_URL}/${SWIFT_BUCKET_NAME}"
